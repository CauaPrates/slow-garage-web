...
| 9 | `VehicleForm` (obrigatórios + `<details>` "mais detalhes" + status) | `src/features/vehicle/VehicleForm.tsx` | AC-2, AC-3, AC-4, AC-8 | 4, 5, 6 | ☑ (grid ajustado pra empilhar abaixo de `sm` — truncamento achado no `ui:check`, ADR-019) |
...
| 19 | Verificação manual completa dos 13 ACs com `alice@dev.local`/`bob@dev.local` | — | todos | 16 | ☑ (AC-1 verificado por revisão de código, não ao vivo — ambas as contas seed já tinham veículo, ver verification.md) |
